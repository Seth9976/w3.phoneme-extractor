...
bitflags!(
    /// Flags for rectangle rounding
    #[repr(C)]
    pub struct ImGuiRoundingFlags: c_int {
        const TopLeft     = 1;
        const TopRight    = 1 << 1;
        const BottomRight = 1 << 2;
        const BottomLeft  = 1 << 3;
        const All         =
            ImGuiRoundingFlags::TopLeft.bits | ImGuiRoundingFlags::TopRight.bits |
            ImGuiRoundingFlags::BottomLeft.bits | ImGuiRoundingFlags::BottomRight.bits;
    }
);
...
impl Default for ImFontConfig {
    fn default() -> ImFontConfig {
        ImFontConfig {
            ...
            oversample_v: 1,
            pixel_snap_h: false,
            glyph_extra_spacing: ImVec2::default(),
            glyph_offset: ImVec2::default(),
            glyph_ranges: std::ptr::null(),
            merge_mode: false,
            rasterizer_flags: 0x00,
            rasterizer_multiply: 1.0,

            name: [0; 32],
            dst_font: std::ptr::null_mut(),
        }
    }
}
...
extern "C" {
    pub fn igItemSize(bb: ImRect, text_offset_y: c_float);
    pub fn igItemAdd(bb: ImRect, id: ImGuiID) -> bool;
}
// --- ImGui internal END
