...
impl<'ui, 'p> Window<'ui, 'p> {
    ...
    #[inline]
    pub fn size_constraints(mut self, min: (f32, f32), max: (f32, f32)) -> Self {
        self.size_min = min;
        self.size_max = max;
        self
    }
    ...
    pub fn build<F: FnOnce()>(self, f: F) {
        let render = unsafe {
            ...
            if self.size_min.0 > 0.0 {
                if (self.size_min.0 - self.size_max.0).abs() < std::f32::EPSILON {
                    sys::igSetNextWindowContentWidth(self.size_min.0);
                }
                sys::igSetNextWindowSizeConstraints(self.size_min.into(),
                                                          self.size_max.into(),
                                                          None,
                                                          ptr::null_mut());
            }
    ...
    pub fn build_modal<F: FnOnce()>(self, f: F) {
        let render = unsafe {
            if !self.pos_cond.is_empty() {
                sys::igSetNextWindowPos(self.pos.into(), self.pos_cond, ImVec2::zero());
            }
            ...
            if let Some(ref opened) = self.opened {
                if **opened {
                    sys::igOpenPopup(self.name.as_ptr());
                }
            }
            sys::igBeginPopupModal(self.name.as_ptr(),
                                    self.opened
                                        .map(|x| x as *mut bool)
