...
pub use sys::{ImDrawIdx, ImDrawVert, ImGuiColorEditFlags, ImGuiHoveredFlags, ImGuiInputTextFlags,
              ...
              ImGuiWindowFlags, ImGuiRoundingFlags, ImVec2, ImVec4};
...
pub struct ImGui {
    ...
    // for keeping ownership of font configs
    fonts: fonts::FontAtlas,
}
...
impl ImGui {
    pub fn init() -> ImGui {
        ImGui {
            ini_filename: None,
            log_filename: None,
            fonts: fonts::FontAtlas::new(unsafe { &*sys::igGetIO() }.fonts),
        }
    }
    ...
    pub fn add_font(&mut self, font: fonts::Font, config: fonts::FontConfig) -> fonts::FontId {
        self.fonts.add_font(font, config)
    }
    ...
    #[inline]
    pub fn mouse_states(&self) -> &[bool; 5] {
        let io = self.io();
        &io.mouse_down
    }
    ...
    #[inline]
    pub fn keys_down(&self) -> &[bool; 512] {
        &self.io().keys_down
    }
...
impl<'ui> Ui<'ui> {
    ...
    pub fn show_style_editor(&self, style: &mut ImGuiStyle) {
        unsafe {
...
impl<'ui> Ui<'ui> {
    ...
    pub fn get_contentregion_width(&self) -> f32 {
        unsafe { sys::igGetWindowContentRegionWidth() }
    }

    pub fn get_contentregion_avail_width(&self) -> f32 {
        unsafe { sys::igGetContentRegionAvailWidth() }
    }

    pub fn with_region_height<F>(&self, id: &ImStr, lines: f32, f: F)
        where F: FnOnce()
    {
        let render = unsafe {
            let line_height = sys::igGetItemsLineHeightWithSpacing() * lines;
            sys::igBeginChild(id.as_ptr(), (0.0, line_height).into(), false, ImGuiWindowFlags::empty())
        };
        if render {
            f();
        }
...
impl<'ui> Ui<'ui> {
    ...
    pub fn text_colored<A>(&self, col: A, text: &ImStr)
    where
        A: Into<ImVec4>,
    {
        unsafe {
    ...
    pub fn button<S: Into<ImVec2>>(&self, label: &ImStr, size: S) -> bool {
        unsafe { sys::igButton(label.as_ptr(), size.into()) }
    }
    pub fn enabled_button<S: Into<ImVec2>>(&self, label: &ImStr, size: S, enabled: bool) -> bool {
        if enabled {
            unsafe { sys::igButton(label.as_ptr(), size.into()) }
        } else {
            unsafe {
                let col_button = sys::igGetColorU32(ImGuiCol::Button, 1.0);
                let col_text = sys::igGetColorU32(ImGuiCol::TextDisabled, 1.0);
                sys::igPushStyleColorU32(ImGuiCol::Text, col_text);
                sys::igPushStyleColorU32(ImGuiCol::ButtonHovered, col_button);
                sys::igPushStyleColorU32(ImGuiCol::ButtonActive, col_button);
                sys::igButton(label.as_ptr(), size.into());
    ...
    pub fn small_enabled_button(&self, label: &ImStr, enabled: bool) -> bool {
        if enabled {
            unsafe { sys::igSmallButton(label.as_ptr()) }
        } else {
            unsafe {
                ...
                sys::igPushStyleColorU32(ImGuiCol::ButtonHovered, col_button);
                sys::igPushStyleColorU32(ImGuiCol::ButtonActive, col_button);
                sys::igSmallButton(label.as_ptr());
                sys::igPopStyleColor(3);
            }
            false
        }
...
impl<'ui> Ui<'ui> {
    ...
    pub fn vslider_int<'p>(&self,
                          label: &'p ImStr,
                          value: &'p mut i32,
                          min: i32,
                          max: i32)
                          -> VSliderInt<'ui, 'p> {
        VSliderInt::new(self, label, value, min, max)
    }
    pub fn vslider_float<'p>(&self,
                            label: &'p ImStr,
                            value: &'p mut f32,
                            min: f32,
                            max: f32)
                            -> VSliderFloat<'ui, 'p> {
        VSliderFloat::new(self, label, value, min, max)
    }
}
...
impl<'ui> Ui<'ui> {
    pub fn selectable<S: Into<ImVec2>>(
        &self,
        label: &ImStr,
        selected: bool,
...
impl<'ui> Ui<'ui> {
    pub fn combo<'p>(
        ...
        let items_inner: Vec<*const c_char> = items.iter().map(|item| item.as_ptr()).collect();
        unsafe {
            sys::igCombo(
                label.as_ptr(),
                current_item,
...
// Widgets: ListBox
impl<'ui> Ui<'ui> {
    pub fn list_box2<'p>(&self, items: &'p [&'p ImStr], current_item: &'p mut i32) -> ListBox<'ui, 'p> {
        ListBox::new(self, items, current_item)
    }
...
impl<'ui> Ui<'ui> {
    ...
    /// }
    /// ```
    pub fn radio_button_bool(&self, label: &ImStr, value: bool) -> bool {
...
impl<'ui> Ui<'ui> {
    ...
    pub fn with_font<F: FnOnce()>(&self, fontid: &fonts::FontId, f: F) {
        if let Some(font) = self.imgui.fonts.font(&fontid) {
